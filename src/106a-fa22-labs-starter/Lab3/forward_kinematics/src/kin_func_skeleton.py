...
def rotation_3d(omega, theta):
    ...
    # def f(w):
    #     return np.power(np.e, w*theta)

    # return np.array([[f(c) for c in r] for r in skew_3d(omega)])

    wh = skew_3d(omega)
    wn = np.linalg.norm(omega)

    return np.identity(3) + wh / wn * np.sin(wn * theta) + (wh @ wh) / np.power(wn, 2) * (1 - np.cos(wn * theta))
    # YOUR CODE HERE
...
def hat_3d(xi):
    ...
    return np.array([[0, -wz, wy, vx], [wz, 0, -wx, vy], [-wy, wx, 0, vz], [0, 0, 0, 0]])
...
def homog_3d(xi, theta):
    ...
    if wx == 0 and wy == 0 and wz == 0:
        return np.array([[1, 0, 0, vx*theta], [0, 1, 0, vy*theta], [0, 0, 1, vz*theta], [0, 0, 0, 1]])
    else:
        right = rotation_3d(omega, theta)
        right = np.array([[r[0] for r in c] for c in right])

        left =  (1 / np.linalg.norm(omega) ** 2) * ((np.identity(3) - right) @ (skew_3d(omega) @ v) + omega @ omega.T @ (v * theta))
        left = np.array([r[0] for r in left])

        return np.array([[right[0,0], right[0,1], right[0,2], left[0]], [right[1,0], right[1,1], right[1,2], left[1]], [right[2,0], right[2,1], right[2,2], left[2]], [0, 0, 0, 1]])
...
def array_func_test(func_name, args, ret_desired):
    ...
    print(ret_value, 'vs', ret_desired)
