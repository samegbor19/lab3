...
def baxter_forward_kinematics_from_angles(joint_angles):
    ...
    R = np.array([[0.0076, 0.0001, -1.0000, qs[0,7]],
                  [-0.7040, 0.7102, -0.0053, qs[1,7]],
                  [0.7102, 0.7040, 0.0055, qs[2,7]], 
                  [0, 0, 0, 1]]).T # rotation matrix of zero config
    #get xi
    xi_ls = []
    for i in range(7): 
        xi_ls.append(np.concatenate((np.cross(-ws[:,i], qs[:,i]), ws[:,i])))

    xi_ls = np.array(xi_ls).T
    g = kfs.prod_exp(xi_ls, joint_angles) @ R
...
if __name__ == "__main__":
    print(baxter_forward_kinematics_from_angles(np.array([0, 0, 0, 1, 2, 3, 0])))